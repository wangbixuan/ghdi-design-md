...
| 组件名 | `BizData` |
| 暴露来源 | `expose.js` |
| importPath | `@/module/admin/BizData/components/detail.vue` |
| 源码路径 | `bigboss-base/src/module/admin/BizData/components/detail.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `8` |
...
业务数据入口/明细组件，用于围绕业务编码展示可点击的数据按钮或数据详情。

源码位于 `bigboss-base/src/module/admin/BizData/components/detail.vue`。扫描到 8 个直接或间接引用样例。
...
- `flowKey`：源码中识别到的入参，使用前应结合业务含义传值。
...
- `bigboss-base/src/components/docIcon.vue`

...
| 组件名 | `actionTitle` |
| 暴露来源 | `expose.js` |
| importPath | `@/components/form/actionTitle` |
| 源码路径 | `bigboss-base/src/components/form/actionTitle.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
表单/详情页中的操作标题或区块标题组件，用来统一标题视觉和操作入口。

源码位于 `bigboss-base/src/components/form/actionTitle.vue`。扫描到 1 个直接或间接引用样例。

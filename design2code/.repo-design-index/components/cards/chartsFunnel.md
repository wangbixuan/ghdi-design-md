...
| 组件名 | `chartsFunnel` |
| 暴露来源 | `expose.js` |
| importPath | `./charts/funnel` |
| 源码路径 | `bigboss-base/src/components/charts/funnel.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
漏斗图 ECharts 封装组件，通过 option 渲染漏斗类图表。

源码位于 `bigboss-base/src/components/charts/funnel.vue`。扫描到 1 个直接或间接引用样例。

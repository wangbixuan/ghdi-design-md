...
| 组件名 | `CarouselVue` |
| 暴露来源 | `expose.js` |
| importPath | `./form/CarouselVue.vue` |
| 源码路径 | `bigboss-base/src/components/form/CarouselVue.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
源码位于 `bigboss-base/src/components/form/CarouselVue.vue`。扫描到 1 个直接或间接引用样例。
...
- `autoPlay`：源码中识别到的入参，使用前应结合业务含义传值。
- `duration`：源码中识别到的入参，使用前应结合业务含义传值。
- `btnSize`：源码中识别到的入参，使用前应结合业务含义传值。
- `activePointColor`：源码中识别到的入参，使用前应结合业务含义传值。
- `inactivePointColor`：源码中识别到的入参，使用前应结合业务含义传值。

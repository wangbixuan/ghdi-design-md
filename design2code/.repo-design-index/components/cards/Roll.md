...
| 组件名 | `Roll` |
| 暴露来源 | `expose.js` |
| importPath | `@/components/bigboss-app/app-infoDrawer/components/table.vue` |
| 源码路径 | `bigboss-base/src/components/bigboss-app/app-infoDrawer/components/table.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
滚动列表展示组件，与 CarouselList 指向同一内部表格实现。

源码位于 `bigboss-base/src/components/bigboss-app/app-infoDrawer/components/table.vue`。扫描到 1 个直接或间接引用样例。

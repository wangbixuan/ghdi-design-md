...
| 组件名 | `PdfCard` |
| 暴露来源 | `expose.js` |
| importPath | `@/components/pdfCard/index.vue` |
| 源码路径 | `bigboss-base/src/components/pdfCard/index.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
PDF 卡片展示组件，通过 pdfList 展示 PDF 列表。

源码位于 `bigboss-base/src/components/pdfCard/index.vue`。扫描到 1 个直接或间接引用样例。
...
- `width`：源码中识别到的入参，使用前应结合业务含义传值。
- `height`：源码中识别到的入参，使用前应结合业务含义传值。
- `pdfList`：源码中识别到的入参，使用前应结合业务含义传值。
- `type`：源码中识别到的入参，使用前应结合业务含义传值。
- `showMyEvaluation`：源码中识别到的入参，使用前应结合业务含义传值。

## 6. 常用事件

- `viewEvent`：组件会向父级抛出的事件。
- `downEvent`：组件会向父级抛出的事件。
- `tomy`：组件会向父级抛出的事件。

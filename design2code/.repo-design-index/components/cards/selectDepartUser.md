...
| 字段 | 值 |
|---|---|
| 组件名 | `selectDepartUser` |
| 暴露来源 | `expose.js` |
| importPath | `./form/selectDepartUser/index.vue` |
| 源码路径 | `bigboss-base/src/components/form/selectDepartUser/index.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
源码位于 `bigboss-base/src/components/form/selectDepartUser/index.vue`。扫描到 1 个直接或间接引用样例。
...
- `selectData`：源码中识别到的入参，使用前应结合业务含义传值。

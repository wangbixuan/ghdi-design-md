...
| 组件名 | `PicPreview` |
| 暴露来源 | `expose.js` |
| importPath | `./PicPreview` |
| 源码路径 | `bigboss-base/src/components/PicPreview.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `2` |
...
源码位于 `bigboss-base/src/components/PicPreview.vue`。扫描到 2 个直接或间接引用样例。
...
- `src`：源码中识别到的入参，使用前应结合业务含义传值。
- `type`：源码中识别到的入参，使用前应结合业务含义传值。
- `validator`：源码中识别到的入参，使用前应结合业务含义传值。
- `copy`：源码中识别到的入参，使用前应结合业务含义传值。
- `del`：源码中识别到的入参，使用前应结合业务含义传值。
...
真实引用样例文件：
- `bigboss-base/src/components/form/image.vue`

...
| 字段 | 值 |
|---|---|
| 组件名 | `DepartmentSelection` |
| 暴露来源 | `expose.js` |
| importPath | `./form/departmentSelection.vue` |
| 源码路径 | `bigboss-base/src/components/form/departmentSelection.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
源码位于 `bigboss-base/src/components/form/departmentSelection.vue`。扫描到 1 个直接或间接引用样例。
...
- `showDepart`：源码中识别到的入参，使用前应结合业务含义传值。
- `type`：源码中识别到的入参，使用前应结合业务含义传值。
- `deptTypeList`：源码中识别到的入参，使用前应结合业务含义传值。

## 6. 常用事件

- `changeType`：组件会向父级抛出的事件。
- `changeDepart`：组件会向父级抛出的事件。
...
<DepartmentSelection
  ...
  @changeDepart="handleChangeDepart"
/>
```

...
| 组件名 | `SelectUser` |
| 暴露来源 | `expose.js` |
| importPath | `../module/admin/SelectUser/index.vue` |
| 源码路径 | `bigboss-base/src/module/admin/SelectUser/index.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `8` |
...
源码位于 `bigboss-base/src/module/admin/SelectUser/index.vue`。扫描到 8 个直接或间接引用样例。
...
- `deleteData`：源码中识别到的入参，使用前应结合业务含义传值。
- `type`：源码中识别到的入参，使用前应结合业务含义传值。
- `keyword`：源码中识别到的入参，使用前应结合业务含义传值。
- `disabled`：源码中识别到的入参，使用前应结合业务含义传值。

## 6. 常用事件

- `selectItem`：组件会向父级抛出的事件。
...
- `bigboss-base/src/components/bigboss-app/app-infoDrawer/module/bottom/index.vue`
- `bigboss-base/src/components/flow/WorkFlowDetail.vue`
- `bigboss-base/src/components/form/iv-autocomplete.vue`

...
| 组件名 | `viewWord` |
| 暴露来源 | `expose.js` |
| importPath | `./form/viewWord/index.vue` |
| 源码路径 | `bigboss-base/src/components/form/viewWord/index.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `2` |
...
Word 文档查看组件，通过 attachTitle 等控制展示。

源码位于 `bigboss-base/src/components/form/viewWord/index.vue`。扫描到 2 个直接或间接引用样例。
...
- `attachTitle`：源码中识别到的入参，使用前应结合业务含义传值。
...
真实引用样例文件：
- `bigboss-base/src/components/hrProfile/index.vue`

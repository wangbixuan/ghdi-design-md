...
| 组件名 | `quarter` |
| 暴露来源 | `expose.js` |
| importPath | `@/components/form/quarter` |
| 源码路径 | `bigboss-base/src/components/form/quarter.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
季度选择表单组件，通过 quarterValue 绑定季度。

源码位于 `bigboss-base/src/components/form/quarter.vue`。扫描到 1 个直接或间接引用样例。
...
- `quarterValue`：源码中识别到的入参，使用前应结合业务含义传值。
- `type`：源码中识别到的入参，使用前应结合业务含义传值。

## 6. 常用事件

- `quarterChange`：组件会向父级抛出的事件。

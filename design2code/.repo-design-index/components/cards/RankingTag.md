...
| 组件名 | `RankingTag` |
| 暴露来源 | `expose.js` |
| importPath | `./form/RankingTag.vue` |
| 源码路径 | `bigboss-base/src/components/form/RankingTag.vue` |
| 说明状态 | `human-reviewed` |
| 推荐状态 | `allowed` |
| 引用样例数量 | `1` |
...
排行标签/角标组件，通过 imageUrl 等展示排行视觉。

源码位于 `bigboss-base/src/components/form/RankingTag.vue`。扫描到 1 个直接或间接引用样例。
...
- `imageUrl`：源码中识别到的入参，使用前应结合业务含义传值。
